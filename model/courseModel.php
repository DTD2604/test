...
{
    $db = connectionDb();
    $sql = "SELECT * FROM `course` WHERE (`name` LIKE :keyword OR `slug` LIKE :slug) AND `deleted_at` IS NULL";
    $stmt = $db->prepare($sql);
    $key = "%{$keyword}%";
    $data = [];
    if ($stmt) {
...
function getAllDataCoursesByPage($keyword = null, $start = 0, $limit = LIMIT_ITEM_PAGE)
{
    ...
    $sql = "SELECT departments.name AS department_name, course.*
            ...
            WHERE (course.`name` LIKE :keywordName OR course.`slug` LIKE :keywordSlug) 
